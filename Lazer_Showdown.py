...
class Button:
    ...
    def is_pressed(self):
        ...
        pressed = (mouse_pressed and self.rect.collidepoint(mouse_pos)) or (
            touch_pos and self.rect.collidepoint(touch_pos)
        )
...
class GameManager:
    ...
    def get_grid_origin(self):
        ...
        return (
            w // 2 - (GRID_SIZE // 2) * CELL_SIZE,
            h // 2 - (GRID_SIZE // 2) * CELL_SIZE,
        )
    ...
    def draw_grid(self):
        ...
        for i in range(GRID_SIZE + 1):
            pygame.draw.line(
                ...
                WHITE,
                (xo, yo + i * CELL_SIZE),
                (xo + GRID_SIZE * CELL_SIZE, yo + i * CELL_SIZE),
                GRID_WIDTH,
            )
            pygame.draw.line(
                self.screen,
                WHITE,
                (xo + i * CELL_SIZE, yo),
                (xo + i * CELL_SIZE, yo + GRID_SIZE * CELL_SIZE),
                GRID_WIDTH,
            )
    ...
    def draw_palette(self):
        ...
        for y in [150, 250, 350, 450, 550, 650]:
            pygame.draw.rect(
                self.screen,
                WHITE,
                (xo, y, PALETTE_BOX_SIZE, PALETTE_BOX_SIZE),
                GRID_WIDTH,
            )
    ...
    def redraw_scene(self):
        ...
        for p in self.state.point_pieces:
            p.draw(self.screen)
        for m in self.state.mirror_pieces:
            m.draw(self.screen)
        for d in self.state.dice_list:
            d.draw(self.screen)
    ...
    def choose_save_file(self):
        ...
        """
        folder = os.path.join(self.save_folder)
            ...
            for i, fname in enumerate(files):
                ...
                if i == idx:
                    pygame.draw.rect(
                        self.screen,
                        (100, 100, 150),
                        pygame.Rect(35, y - 2, panel.width - 30, 28),
                        width=2,
                        border_radius=4,
                    )
...
class Piece:
    ...
    def update_position_from_grid(self):
        ...
        if self.grid_position:
            ...
            self.rect = pygame.Rect(
                xo + self.grid_position[0] * CELL_SIZE,
                yo + self.grid_position[1] * CELL_SIZE,
                CELL_SIZE,
                CELL_SIZE,
            )
        else:
            self.rect = pygame.Rect(
                self.palette_position[0], self.palette_position[1], CELL_SIZE, CELL_SIZE
            )
    ...
    def snap_to_grid(self, occupied):
        ...
        """
        xo, yo = GameManager.get_grid_origin_static()
        gx = (self.rect.x - xo + CELL_SIZE // 2) // CELL_SIZE
        gy = (self.rect.y - yo + CELL_SIZE // 2) // CELL_SIZE
        if 0 <= gx < GRID_SIZE and 0 <= gy < GRID_SIZE and (gx, gy) not in occupied:
...
class lazerPiece(Piece):
    ...
    def __init__(self, x, y):
        ...
        self.lzrBeamImg = pygame.transform.scale(
            pygame.image.load(PATH + "assets/images/lzrBeamImg.png"),
            (CELL_SIZE, CELL_SIZE),
        )
        self.lzrBeamstrtImg = pygame.transform.scale(
            pygame.image.load(PATH + "assets/images/lzrBeamstrtImg.png"),
            (CELL_SIZE, CELL_SIZE),
        )
    ...
    def rotate_img_direction(self):
        ...
        self.image = pygame.transform.rotate(self.og_img, ang)
    ...
    def fire_laser(self, gs):
        ...
        if not self.grid_position:
            return
        path = self.calculate_laser_path(gs)
        self.apply_laser_effects(gs, path)
    ...
    def calculate_laser_path(self, gs):
        ...
        xo, yo = GameManager.get_grid_origin_static()
        path = []
        dx, dy = self._dir_to_delta(d)
        x += dx
        y += dy
        while 0 <= x < GRID_SIZE and 0 <= y < GRID_SIZE:
            path.append(
                (
                    xo + x * CELL_SIZE + CELL_SIZE // 2,
                    yo + y * CELL_SIZE + CELL_SIZE // 2,
                )
            )
            for m in gs.mirror_pieces:
                if m.grid_position == (x, y):
                    d = self.reflect_laser(d, m.mirror_type)
                    dx, dy = self._dir_to_delta(d)
                    break
            else:
                for p in gs.point_pieces:
                    if p.grid_position == (x, y):
                        gs.score += p.value
                        gs.point_pieces.remove(p)
                        return path
            x += dx
            y += dy
        return path

    def apply_laser_effects(self, gs, path):
        pass
    ...
    def draw_laser_path(self, path):
        ...
        for i in range(len(path) - 1):
            s = path[i]
            e = path[i + 1]
            v = pygame.math.Vector2(e[0] - s[0], e[1] - s[1])
            ang = v.angle_to(pygame.math.Vector2(1, 0))
            spr = pygame.transform.rotate(self.lzrBeamImg, ang)
            surface = pygame.display.get_surface()
            surface.blit(spr, pygame.Rect(spr.get_rect(center=s).topleft, ()).topleft)
            if i == 0:
                st = pygame.transform.rotate(self.lzrBeamstrtImg, ang)
                surface.blit(st, st.get_rect(center=s).topleft)
            pygame.display.flip()
            pygame.time.delay(100)
    ...
    def reflect_laser(self, d, mt):
        ...
        """
        if mt == "/":
            return {"up": "left", "down": "right", "left": "up", "right": "down"}[d]
...
class pointPiece(Piece):
    ...
    def get_image_path(self, v):
        ...
        """
        return {
            20: "assets/images/pntImg20.png",
            30: "assets/images/pntImg30.png",
            50: "assets/images/pntImg50.png",
        }.get(v, "assets/images/pntDefImg.png")
...
class mirrorPiece(Piece):
    ...
    def draw(self, screen):
        ...
        if self.mirror_type == "/":
            ...
            screen.blit(
                pygame.transform.flip(self.og_img, True, False), self.rect.topleft
            )
    ...
    def __init__(self, x, y):
        ...
        self.images = [
            pygame.transform.scale(
                pygame.image.load(PATH + f"assets/images/dice/{i}.png"),
                (CELL_SIZE, CELL_SIZE),
            )
            for i in range(1, 7)
        ]
...
def main_game_loop(screen):
    ...
    while running:
        mgr.redraw_scene()
        occupied = {p.grid_position for p in state.get_all_pieces() if p.grid_position}
        for e in pygame.event.get():
            if e.type == pygame.QUIT:
                running = False
                ...
                if restartBtn.is_pressed():
                    ...
                    for p in reversed(state.get_all_pieces()):
                        if p.rect.collidepoint(e.pos):
                            ...
                            draggable = p
                            p.dragging = True
                            offx = e.pos[0] - p.rect.x
                            offy = e.pos[1] - p.rect.y
                            break
            elif e.type == pygame.MOUSEBUTTONUP and draggable:
                mgr.save_state()
                draggable.snap_to_grid(occupied)
                draggable.dragging = False
                if isinstance(draggable, mirrorPiece) and draggable.grid_position:
                    mt = draggable.mirror_type
                    po = (
                        screen.get_width() // 2 + (GRID_SIZE // 2) * CELL_SIZE
                    ) + CELL_SIZE
                    yo = 550 if mt == "/" else 650
                    state.mirror_pieces.append(mirrorPiece(po, yo, mt))
                draggable = None
                ...
                if e.key == pygame.K_z:
                    ...
                    mgr.save_state()
                    [d.roll() for d in state.dice_list]
